...
update_option("jr_youtube_link_personal", $anch);
}
}
...
$update=get_option('jr_youtube_links_choice');
$update=str_replace("Sponsored by", "Powered by", $update);
$update=jr_removeBOM($update);
update_option('jr_youtube_links_choice', $update);

$youtubedisp .= "<p style='font-size:x-small'>YouTube Plugin created by ".$linkper." - ".get_option('jr_youtube_links_choice')."</p>";
}
...
if (get_option("jr_youtube_rescue")=="") {
$pie=explode("href", get_option('jr_youtube_links_choice'));

$pie[0]="Powered by <a ";

$bacon=implode("href", $pie);
update_option('jr_youtube_links_choice', $bacon);
update_option('jr_youtube_rescue', '1');
}
