...
Description: Displays your recent youtube videos as a widget.
Version: 1.1.0
Author: Jake Ruston
...
function jr_youtube_options_page() {
	...
	$data_field_name_4 = 'mt_youtube_choice';
...
<?php
	...
	$change5 = get_option("mt_youtube_choice");
...
<p><?php _e("Show User Videos or Search Result Videos?", 'mt_trans_domain' ); ?> 
<input type="radio" name="<?php echo $data_field_name_4; ?>" value="user" <?php echo $change5; ?>>User
<input type="radio" name="<?php echo $data_field_name_4; ?>" value="search" <?php echo $change51; ?>>Search
</p><hr />
...
function show_youtube() {
  ...
  $optionyoutubecache = get_option("mt_youtube_cache");
  $youtubechoice = get_option("mt_youtube_choice");
...
$youtubedisp="";
  ...
  $youtubedisp .= '<div id="recent-posts-4" class="widget widget_recent_entries"><h3>'; 

  $youtubedisp .= $widget_title.'</h3><ul>';
...
$url = "http://www.youtube.com/watch?v=".$ending;
    ...
    $youtubedisp .= '<li><font color="#000000" size="2"><a href="'.$url.'">'.$title.'</a></font></li>';
 
    if($i++ >= $max_tracks) break;
  }

if ($supportplugin=="Yes" || $supportplugin=="") {
$youtubedisp .= "<p style='font-size:x-small'>Plugin created by <a href='http://www.jakeruston.co.uk'>Jake Ruston</a>.</p>";
}
...
register_sidebar_widget("JR YouTube", "show_youtube");
}
